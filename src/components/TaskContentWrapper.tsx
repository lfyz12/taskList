...
const TaskContentWrapper = () => {
  ...
  const handleImproveText = useCallback((newText: string) => {
    if (task) {
      setText(newText)
      taskStore.updateTask(task.id, {text: newText})
    }
  }, [task, taskStore])

  const handleSplitSubtasks = useCallback((names: string[]) => {
    if (!task) return
    names.forEach(n => {
      taskStore.createNewTask(n, '')
    ...
    <div className='flex flex-col h-full p-4 md:p-8 pt-14 lg:pt-8'>
      ...
      <div className='flex items-start gap-2 mb-2'>
        ...
        <button
          ...
          title={taskStore.aiApiKey ? undefined : taskStore.t('ai.noKey')}
          className='shrink-0 flex items-center gap-1 px-2.5 py-1.5 rounded-apple text-[11px] font-semibold cursor-pointer transition-all
            bg-apple-blue/8 text-apple-blue hover:bg-apple-blue/15
            dark:bg-apple-blue/15 dark:text-apple-blue-300 dark:hover:bg-apple-blue/25'
        >
      ...
      {showAI && (
        <div className='mb-4 md:mb-5'>
          <AIActionPanel
            taskName={task.name}
            taskText={text}
            apiKey={taskStore.aiApiKey}
            t={taskStore.t.bind(taskStore)}
